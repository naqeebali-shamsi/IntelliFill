...
export class DOCXParsingStrategy extends BaseParsingStrategy {
  ...
  async parse(pathOrUrl: string): Promise<ParsedDocument> {
    try {
      logger.info(`Parsing DOCX: ${pathOrUrl}`);
      ...
      // Extract metadata
      const metadata = await this.extractMetadata(pathOrUrl, textResult);
  ...
  private async extractMetadata(filePath: string, textResult: any): Promise<any> {
    ...
    return {
      ...
      hasImages: textResult.messages?.some(
  ...
  private extractStructuredData(html: string): Record<string, any> {
    ...
    if (headingMatches) {
      structuredData.headings = headingMatches.map((h) => h.replace(/<[^>]+>/g, '').trim());
    }
