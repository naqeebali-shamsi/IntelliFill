---
title: 'Instrumentation & Observability Strategy'
description: 'Comprehensive instrumentation strategy for IntelliFill observability and compliance'
category: 'reference'
...
export class PIISanitizer {
  ...
  private static isPIIField(fieldName: string): boolean {
    ...
    return Array.from(this.PII_FIELD_PATTERNS).some((pattern) => lower.includes(pattern));
  }
  ...
  private static sanitizeString(str: string): string {
    ...
    // Emirates ID
    str = str.replace(/\b784-\d{4}-\d{7}-\d{1}\b/g, '[EMIRATES_ID_REDACTED]');
    return str;
  }
}
```
