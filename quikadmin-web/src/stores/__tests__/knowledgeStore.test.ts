...
describe('useKnowledgeStore', () => {
  ...
  describe('fetchSources', () => {
    ...
    it('sets error on failure', async () => {
      vi.mocked(knowledgeService.getKnowledgeSources).mockRejectedValue(
  ...
  describe('uploadSource', () => {
    it('uploads and adds source to list', async () => {
      ...
      const result = await useKnowledgeStore.getState().uploadSource(
    ...
    it('tracks upload progress', async () => {
      ...
      const uploadPromise = useKnowledgeStore.getState().uploadSource(
        new File(['content'], 'test.pdf'),
        'Test'
      )
  ...
  describe('search', () => {
    ...
    it('sets error on search failure', async () => {
      vi.mocked(knowledgeService.semanticSearch).mockRejectedValue(
  ...
  describe('hybridSearch', () => {
    it('performs hybrid search', async () => {
      const mockResults = [
        { id: 'r1', text: 'Result', vectorScore: 0.8, keywordScore: 0.9 },
      ]
  ...
  describe('fetchFieldSuggestions', () => {
    it('fetches and stores field suggestions', async () => {
      ...
      expect(useKnowledgeStore.getState().fieldSuggestions.firstName).toEqual(
        mockSuggestions
      )
    })

    it('returns empty array on failure', async () => {
      vi.mocked(knowledgeService.getFieldSuggestions).mockRejectedValue(
        new Error('Failed')
      )
  ...
  describe('fetchFormSuggestions', () => {
    it('fetches suggestions for multiple fields', async () => {
      ...
      vi.mocked(knowledgeService.getFormSuggestions).mockResolvedValue(
        mockResponse as any
      )
