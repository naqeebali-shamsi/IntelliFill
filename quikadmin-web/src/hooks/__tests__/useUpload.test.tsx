...
describe('useUpload Hook', () => {
  ...
  describe('Upload Initiation', () => {
    it('starts uploads when autoStart is true', async () => {
      ...
      vi.mocked(api.uploadDocuments).mockImplementation(async () => {
  ...
  describe('Retry Logic', () => {
    it('retries failed uploads when retryOnError is true', { timeout: 15000 }, async () => {
      ...
      const { result } = renderHook(() => useUpload({ autoStart: true, retryOnError: true }), {
        wrapper,
      });
      ...
      await waitFor(
        () => {
          expect(api.uploadDocuments).toHaveBeenCalledTimes(2);
        },
        { timeout: 3000 }
      );
    });
  ...
  describe('Progress Tracking', () => {
    it('updates progress during upload', async () => {
      ...
      vi.mocked(api.uploadDocuments).mockImplementation((formData, onProgress) => {
        // Simulate progress
        if (onProgress) {
          onProgress(50);
          onProgress(100);
  ...
  describe('Concurrent Uploads', () => {
    it('limits concurrent uploads to maxConcurrent', { timeout: 15000 }, async () => {
      ...
      const { result } = renderHook(() => useUpload({ autoStart: true, maxConcurrent: 3 }), {
        wrapper,
      });
